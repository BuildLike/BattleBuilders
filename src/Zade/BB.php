...
class BB extends PluginBase implements Listener {
	...
	{
		$this->getLogger()->notice(TE::AQUA . "Battle Builders is created by Zadezter!");
            ...
            if(in_array($level,$this->arenas))
            {
                ...
                if($time<=170)
                {
                    if($config->get("actual".$level)!=$player->getName())
                    {
                        ...
                        {
                            $player->sendTip(TE::BOLD.TE::RED."NAH!");
                        }
                        ...
                        {
                            $player->sendTip(TE::BOLD.TE::DARK_PURPLE."AWESOME!");
                        }
                    ...
                    {
                        $player->sendTip(TE::BOLD.TE::RED."You cannot vote at your own plot!");
                    }
                }
            }
        }
			...
			case "bb":
                                ...
                                if(!empty($args[0]))
				{
                                    if(file_exists($this->getServer()->getDataPath() . "/worlds/" . $args[0]))
                                    {
                                            ...
                                            $this->mode = 1;
                                            $player->sendMessage($this->prefix . "Welcome To Battle Builders setup! You need to register 16 spawn plots and 1 waiting lobby!");
                                    ...
                                    {
                                            $player->sendMessage($this->prefix . "Sorry, this world is missing.");
                                    }
                                }
                            }
			return true;
	...
	public function onInteract(PlayerInteractEvent $event)
	{
		...
		if($tile instanceof Sign) 
		{
			...
			{
				$tile->setText(TE::AQUA . "§a- Join -",TE::GREEN  . "§e0 / 16","§f" . $this->currentLevel,$this->prefix);
				$this->refreshArenas();
				$this->currentLevel = "";
				$this->mode = 0;
				$player->sendMessage($this->prefix . "You have successfully registered the arena! All setup is done!");
                                array_shift($this->op);
			}
			else
			{
				...
				if($text[3] == $this->prefix)
				{
					if($text[0]==TE::AQUA . "§a- Join -")
					{
						$config = new Config($this->getDataFolder() . "/config.yml", Config::YAML);
                                                ...
                                                with:
                                                $slots->save();
                                                $player->sendMessage($this->prefix . "You have entered the BuildBattle game zone!");
                                                foreach($level->getPlayers() as $playersinarena)
                                                {
                                                $playersinarena->sendMessage($player->getNameTag() .TE::AQUA. " joined!");
                                                }
					...
					{
						$player->sendMessage($this->prefix . "This game is already started! Please wait until ends!");
					}
				}
			}
		}
		...
		elseif(in_array($player->getName(), $this->op)&&$this->mode==17)
		{
			$config = new Config($this->getDataFolder() . "/config.yml", Config::YAML);
			$config->set($this->currentLevel . "Spawn" . $this->mode, array($block->getX(),$block->getY()+1,$block->getZ()));
			$player->sendMessage($this->prefix . "You have successfully registered a waiting lobby zone!");
					...
					else
					{
						...
						if($config->get($arena . "start")==1)
						{
							if($timeToStart>0)
							{
								...
								{
									$pl->sendTip(TE::WHITE."This game starts in ".TE::GREEN . $timeToStart . TE::RESET);
                                                                        if($timeToStart<=5)
                                                                        {
							...
							else
							{
								...
								if($time == 469)
								{
									...
									foreach($playersArena as $pl)
									{
                                                                            $pl->sendMessage(TE::YELLOW.">--x-----x------xx------x-----x--<");
                                                                            $pl->sendMessage(TE::YELLOW."> ".TE::BOLD.TE::GREEN."§fBattle".TE::LIGHT_PURPLE." §cBuilders");
                                                                    ...
                                                                    if($time==11 || $time==21 || $time==31 || $time==41 || $time==51 || $time==61 || $time==71 || $time==81 || $time==91 || $time==101 || $time==111 || $time==121 || $time==131 || $time==141 || $time==151 || $time==161)
                                                                    {
                                                                        ...
                                                                        foreach ($playersArena as $pl)
                                                                        {
                                                                            ...
                                                                            if($yo!=$pl->getName())
                                                                            {
                                                                                ...
                                                                                $points->save();
                                                                                $pl->sendMessage($this->prefix .TE::YELLOW. "Vote Confirmed: ".TE::BOLD.$voto);
                                                                            }
                                                                        }
                                                                    }
                                                                    if($time==170)
						...
						else
						{
                                                    foreach($playersArena as $pl)
                                                    {
                                                            $pl->sendTip(TE::YELLOW . "Waiting For Players..." .TE::RESET);
                                                    }
                                                    $config->set($arena . "PlayTime", 470);
                                                    $config->set($arena . "StartTime", 30);
						}
